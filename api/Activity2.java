...
public class Activity2 {
    ...
    public void getUserInfo() {
        ...
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "Hima") // Pass request body from file
        ...
        response.then().body("username", equalTo("Hima"));
        response.then().body("firstName", equalTo("Hima"));
        response.then().body("lastName", equalTo("Bindu"));
        response.then().body("email", equalTo("hb@mail.com"));
    ...
    public void deleteUser() throws IOException {
        Response response =
                given().contentType(ContentType.JSON) // Set headers
                        .pathParam("username", "Hima") // Add path parameter
                        .when().delete(ROOT_URI + "/{username}"); // Send POST request

        // Assertion
        response.then().body("code", equalTo(200));
        response.then().body("message", equalTo("Hima"));
    }
}
